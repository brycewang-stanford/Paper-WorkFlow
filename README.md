...
**Stage 0（Intake & Setup）在最前面静默完成**：建工作区 · 判入口 · 问档位 · 写状态文件。
...
| 物料 | 内容 | 打开 |
|---|---|---|
| 📊 **流程讲义** | 30 页 PPTX / PDF，从选题到投稿的端到端流水线 | [PPTX](社科实证论文工作流.pptx) · [PDF](社科实证论文工作流.pdf) |
| 🧪 **DiD 演示** | 22 cells，一键跑通双重差分基准 + 事件研究 + 稳健性 | [`did_demo.ipynb`](did_demo.ipynb) |
| 🐼 **Pandas 速成** | 38 段代码覆盖 30 个数据处理核心功能 | [`pandas_30_core_features.ipynb`](pandas_30_core_features.ipynb) |

> 图与表均由 [`build_notebook.py`](build_notebook.py) / [`build_pptx.py`](build_pptx.py) 自动生成，可一键复现。
...
│
│   —— 以下为配套演示物料 ——
├── 社科实证论文工作流.pptx · .pdf      # 30 页流程讲义
├── did_demo.ipynb                    # DiD 快速演示 Notebook
├── pandas_30_core_features.ipynb     # Pandas 30 个核心功能
