...
仓库内附一套开箱即用的教学 / 汇报物料：用 30 页讲义把整条工作流讲清楚，再用一个 Notebook 把其中「计量估计」这一步真正跑给观众看。
...
它会检查本地 Markdown 链接、`workflow_state` schema、`init_workspace.sh` 的拒绝覆盖行为、核心资产与 DiD Notebook 结构。母仓库发布前再从仓库根目录跑 `make check`。
