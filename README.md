...
![State](https://img.shields.io/badge/state-schema%20v6-64748B?style=flat-square)
...
| ④ 估计 | Stage 3 | 反事实从哪里来，样本/estimand 与最低诊断证据是否齐全 | **三后端分析路由**：Python/StatsPAI（默认 MCP 优先）/ Stata `.do` / R `fixest`+Quarto + `did-analysis` 等 → `analysis_backend.md`、`sample_audit.md`、`design_register.md`、`method_gate.md` |
...
| Evidence Layer | 数据、样本/estimand 审计、识别设计、分析后端、估计、稳健性、方法证据包 | `analysis_backend.md`、`sample_audit.md`、`design_register.md`、`method_gate.md`、`main_results.json`、`robustness/` |
...
│   ├── workflow_state.template.json  # 进度状态文件模板（v6：含 empirical_audit + analysis_backend + method_gate + quality_gate + replication_pack）
