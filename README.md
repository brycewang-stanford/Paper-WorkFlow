...
| ④ 估计 | Stage 3 | 反事实从哪里来，最低诊断证据是否齐全 | `did-analysis` / `iv-estimation` / `rdd-analysis` / `synthetic-control` 等 → `design_register.md`、`method_gate.md` |
| ⑤ 呈现 | Stage 4 | 审稿人能否一眼读懂结果和识别直觉 | `table`、`figure` → 三线表、事件研究图、系数图 |
...
| 计量估计 | `ols-regression`、`panel-data`、`iv-estimation`、`did-analysis`、`rdd-analysis`、`synthetic-control`、`time-series`、`ml-causal`、`stata`、`stats` | Stage 3，按识别策略生成证据包 |
| 表与图 | `table`、`figure` | Stage 4，生成出版级 exhibits |
