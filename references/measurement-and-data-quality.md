...
|---|---|---|
| **自变量 X**（核心回归元） | classical（与真值独立、均值 0） | **衰减偏误**：系数偏向 0，bias 因子 = reliability ratio λ = Var(X\*)/Var(X)；多元回归中偏误**会外溢**到其它系数，符号不保证 |
...
| **处理 D（二元）** | 误分类（misclassification） | 衰减 + 在 DiD 里若**处理时点**测错（误记 first-treated cohort、漏记退出），偏误可能很严重 |
...
- **变换的坑**：
  ...
  - **asinh / IHS** 常被当作「能处理 0 的 log」滥用——Chen & Roth (2023) 指出其弹性解释**依赖测量单位**、
...
- **选择性缺失/流失的界**：differential attrition 或 selective missingness 无法消除时，给 **bounds** 而非装作点识别——
  Lee (2009) 单调选择 trimming bounds、Manski / Horowitz-Manski worst-case bounds。路由
...
- 测量误差综述与后果：Bound, Brown & Mathiowetz, "Measurement Error in Survey Data," *Handbook of Econometrics* Vol. 5 (2001)；
  Schennach, "Recent Advances in the Measurement Error Literature," *Annual Review of Economics* (2016)。
- 指数合成：Anderson, "Multiple Inference and Gender Differences...," *JASA* 103 (2008)（inverse-covariance 加权指数）。
- 变换尺度依赖：Chen & Roth, "Logs with Zeros? Some Problems and Solutions," *QJE* 139(2) (2024)（asinh/IHS 单位敏感）。
- 选择性流失的界：Lee, "Training, Wages, and Sample Selection," *REStud* 76 (2009)（trimming bounds）；
