...
### CSMAR / WIND / RESSET / CNRDS（中国）→ 见 [`china-data-sources.md`](china-data-sources.md)
- 中国资本市场与公司财务主力库的**权威详版卡片**（覆盖范围、许可、链接键、
  跨库口径陷阱、公司名 merge 标准化、触发的设计风险）统一维护在
  [`china-data-sources.md`](china-data-sources.md)——单一真源，避免双处漂移。
- 速记：firm-year A 股面板；订阅制、再分发受限（复现包给代码 + 股票代码列表，
  不给原始面板）；核心风险 = 跨库口径不一 + 中文公司名 merge 误配 + 仅上市样本选择。
...
- **中国家户库详版**（CFPS/CHFS/CHARLS/CHNS 的申请路径、模块结构、抽样设计与逐库陷阱）：见 [`china-data-sources.md`](china-data-sources.md)。
