...
先在 proposal 与评分卡中声明 `causal`、`descriptive` 或 `predictive` 研究目的，以及学科/目标读者。
此评分不是顶刊录用概率。描述性研究审查目标总体、抽样/权重、测量和不确定性；预测研究审查
独立验证、泄漏、校准和适用范围；因果研究才要求对应识别假设与设计诊断。保留七维结构，
但维度②按所声明的推断任务评分，不强迫所有社会科学研究虚构处理组或因果机制。

空结果、重复研究或与先验相反的结果本身不是失败；贡献评判须看信息增量、精度、可复现性和
目标期刊的实际定位，不能为了通过评分更换结果。尚无证据支持的机制不得为补齐维度而杜撰。
只具备自审能力时记录 reviewer_mode=self_review；不能将一次角色扮演称为独立同行评审。
