...
| `builtin` | 没装 pandoc | 纯标准库，零依赖，任何机器都能出稿 | 只用于简单正文与表图；未解析引用或公式会失败，应使用 Pandoc |
...
Markdown 路径只展开本 skill 的表图 include，保留其余结构给 Pandoc：列表、脚注、元数据、公式和引用。
存在 `.bib` 就启用 citeproc，不以是否指定 CSL 为前提；未知引用或转换警告需修复，不能静默清除 key。
复杂 LaTeX、自定义宏和交叉引用必须检查转换结果；不能保证支持时保留原稿并报告具体未解析项。

输出先在临时文件中构建，成功后再替换目标；失败返回非零并保留原有 DOCX 与状态，不静默降为另一转换器。
成功组装记录 `manuscript.assembly_inputs`（源稿、引用、表图、样式的 SHA-256）和 `assembled_docx_sha256`。
源稿、表图、参考文献或成品修改后 freshness 检查会失败；Word 手工修订须先合并回权威源稿再重建。
哈希保证检测修改，不证明结果真实，也不替代学术审阅。

**逐页视觉验收**：使用实际可用的 Word/LibreOffice 或文档渲染工具生成 PDF/页面图；检查所有页面的表头、
脚注、公式、中文字体、分页、图像和引用。保存渲染命令、页数和审阅发现到 `09_submission/docx_review.md`。
没有渲染能力时明确“结构通过，版面未验证”，不得把结构 checker 的绿灯表述为已完成视觉审阅。
