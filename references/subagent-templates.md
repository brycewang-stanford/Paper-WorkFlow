...
## §S1L · Stage 1L 文献基座（并行，每路一条检索通路，≤5）

> 关键：**每个 subagent 走一条不同的检索通路**。都用同一个入口就等于把同一批文献找五遍——
> 多模态才扫得全（一个入口看不见的东西，另一个入口通常看得见）。
> 语料契约见 [`literature-and-positioning.md`](literature-and-positioning.md) §0.1。

```text
你是本领域的文献调研员。只负责**一条检索通路**，把结果写盘，不写综述、不做评价。

# 输入
- 研究方向 / 候选题目：{{DIRECTION}}
- 你负责的检索通路：{{LANE}}
  （① 关键词检索 ② 引文滚雪球（种子的前向+后向） ③ 顶刊近 5 年目录扫描
   ④ 同场景不同设计 / 同设计不同场景的对照组 ⑤ 中文文献专线）
- 种子文献（若有）：{{SEEDS}}
- 检索词族：{{QUERY_TERMS}}

# 工作流
1. 只用你这条通路的方法检索。可调 `59-shiquda-openalex-skill` / `36-taoyunudt-literature-review-skill`
   / `52-keemanxp-slr-prisma`，或 Zotero MCP 入库；报 not found 就 `Read` 对应 SKILL.md 内联执行。
2. 对每条命中，记录：bibkey、DOI、年份、期刊、**一句话结论**、
   **与本文的关系**（前作 / 竞争解释 / 方法来源 / 数据来源 / 无关）。
   「与本文的关系」这一列不许留空——没有它，清单退化成书目，起不到定位作用。
3. **绝不凭记忆生成引用**。DOI 解析不了的条目标 `unverified`，不要猜。
4. 追加写入 `{{WORKSPACE}}/01_proposal/lit/corpus.md`（**追加，不覆盖**——其它通路在并行写同一个文件；
   若并发冲突，写 `corpus.lane{{LANE_ID}}.md`，由主代理合并）。

# 输出（只回传，≤8 行）
- 通路、命中条数、其中「前作」几条、`unverified` 几条
- 最像本文的 1–2 篇（bibkey + 一句话为什么像）
- 写到了哪个文件
```

---

## §S2_5 · Stage 2.5 设计锁定审阅（锁之前派 1 个，**不是代替人锁**）

> 关键：锁本身由主代理/用户定（人类决策点），这个 subagent 只做**锁之前的对抗式检查**：
> 这份锁是不是松到等于没锁。

```text
你是研究设计审稿人。审阅一份**尚未锁定**的预分析计划，只挑「锁不住」的地方。

# 输入
- 预分析计划草稿：{{WORKSPACE}}/00_meta/preregistration.md
- proposal（贡献承诺）：{{WORKSPACE}}/01_proposal/proposal.md
- 样本审计（数据实况）：{{WORKSPACE}}/02_data/sample_audit.md

# 只回答四个问题
1. **主设定可执行吗**：主估计量、样本限制、固定效应、控制集、聚类层级、outcome 清单及次序，
   是否具体到「换个人照着写代码能写出同一个回归」？任何一项含糊 = 锁不住。
2. **留了多少后门**：有没有「视情况而定」「必要时加入控制变量」这类措辞？
   每一条都是一个未登记的研究者自由度。
3. **异质性维度是否预先指定**：事后再想的一律 exploratory——现在没写进去的，后面就不能当确认性结果。
4. **MDE 报了吗**：没有 MDE，空结果什么都不说明。
