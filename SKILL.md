...
> **🏁 核心交付里程碑 = 「可投稿级初稿」。** Stage 0–7 跑完即产出一份**结构完整、识别可信、
> 表图齐备、语言无 AI 味**的初稿——这就是本编排器对用户的首要承诺。跨过 Stage 7 后**必须先过
> 「初稿质量门」**（见下文「初稿质量门」节）：用独立 critic subagent 按顶刊标准给初稿打分，达标
> 才算「高质量初稿」完成、才进入可选的 Stage 8–9（模拟评审 / 投稿）。**未达标则按质量门的回退
> 指令退回对应阶段重做**，绝不把「跑完流程」当成「达到质量」。
