...
> [`references/research-grade-methods.md`](references/research-grade-methods.md)，「Stage 3–4 的统一估计 +
...
| **3** | 计量识别、估计与方法闸门 | **StatsPAI 引擎**（MCP 优先：`detect_design→preflight→recommend→fit(as_handle)→audit_result→sensitivity_from_result→bibtex`）为主线，按设计配 `did-analysis` / `iv-estimation` / `rdd-analysis` / `synthetic-control` / `panel-data` / `ols-regression` / `time-series` / `ml-causal` + research-grade methods pack | `03_analysis/` 代码 + `design_register.md` + `method_gate.md` |
| **4** | 表与图 | **StatsPAI 出表栈**（`statspai` 包：`regtable`/`paper_tables`/`collect` → Word/Excel/LaTeX 同出 + 标准图谱）为主，`table` + `figure` 为替代 | `04_results/*.{tex,docx,xlsx}` + `*.pdf/png` |
...
> 最低诊断证据、稳健性矩阵与复现脚本齐了，才能把结果送进表图和写作。Stage 7 跑完再过
...
3. **建工作区**。在用户指定目录（缺省为当前工作目录）下创建
   ...
   `00_meta/workflow_state.json` 与 `00_meta/intake.md`。若同名目录已存在，**另建新目录、
   绝不覆盖**。完整布局见
   [`references/workspace-and-state.md`](references/workspace-and-state.md)。关键交付物优先从
   [`templates/`](templates/) 实例化（设计注册、方法闸门、质量评分卡、数据治理、DAS、复现 README、
   投稿 checklist、最终报告），不要临场自创格式。
...
4. **一次性问清三件套**（用一个 `AskUserQuestion`，多选/多题，避免来回打断）：
   ...
   若用户明确说“全自动 / 不要讨论 / 不要中途问我”，或 `$ARGUMENTS` 已足够推断三件套，则按保守缺省
   自动填值（`mode=auto` 或用户指定档位、`target_journal=TBD-by-stage1`、`language` 依输入语言推断），
   并把假设写入 `00_meta/intake.md` 与 `workflow_state.json.decisions`；不要为了偏好问题阻断开跑。

5. **填写状态文件** `00_meta/workflow_state.json`（由 init 脚本从
   ...
   并把每个 Stage 置为 `pending|in_progress|done|skipped`；`method_gate`、`quality_gate` 与
   `replication_pack` 起始置 `pending`（字段含义见
...
7. **不要为「计划」单独求批准**。Setup 完成后，**直接建任务跟踪并开跑**（规划阶段
...
与 [`references/statspai-analysis.md`](references/statspai-analysis.md)（StatsPAI 引擎：MCP 优先拍板+拟合+诊断，
`statspai` 包做出版级出表；其七块稳健性闸门正是下面最低证据包的实现入口），并完成：

1. **设计注册**：写 `03_analysis/design_register.md`，明确 estimand、处理定义、比较组、识别假设、
   主估计量、替代估计量和失败回退。
2. **最低证据包**：按方法分支补齐必需 artifact。交错 DiD 需 CS/SA/BJS 等 group-time 或事件研究稳健
...
3. **方法闸门报告**：写 `03_analysis/method_gate.md`，逐项列出必需证据是否存在、路径在哪里、是否
   `PASS`。若 `NOT PASS`，不得进入 Stage 4；必须按报告回 Stage 1/2/3 修设计、数据或估计。
4. **治理与透明度 hard flags**：同步检查
...
5. **写入状态**：更新 `workflow_state.json.method_gate` 与 `decisions`，记录主设计、主估计量、缺失
   artifact、是否放行。
...
- **绝不伪造数据 / 结果 / 文献**。引用核验交给 `reference-verify` / StatsPAI `bibtex`（`paper.bib` 为唯一
  真源）；数据来源交给 `data-fetcher`；计量结论以真实运行结果为准。Stage 3–4 的统一引擎是 **StatsPAI**
  （MCP 链路 `detect_design → preflight → recommend → fit(as_handle) → audit_result →
  sensitivity_from_result → bibtex`；要出版级三格式表图与 8 段 paper bundle 时切到 `statspai` 包的
  `regtable`/`paper_tables`/`collect`），细则见 [`references/statspai-analysis.md`](references/statspai-analysis.md)。
- **绝不贴空方法标签**。DiD / IV / RDD / SDID / DML / causal forest 等标签必须对应
  `research-grade-methods.md` 要求的证据包；缺 `method_gate.md` 或闸门未过，就不得把相关结果写成主
  因果发现。
...
- **运行与治理横切层**：[`references/runtime-fallbacks.md`](references/runtime-fallbacks.md)
  ...
  公开复现包边界）。
- [`templates/`](templates/) — 可复制到工作区的关键 artifact 模板：design register、method gate、
