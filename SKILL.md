...
> [`references/research-grade-methods.md`](references/research-grade-methods.md)，「Stage 3–4 的 Python/Stata/R
...
| **3** | 计量识别、估计与方法闸门 | **分析后端路由**：默认 Python/StatsPAI（MCP 优先：`detect_design→preflight→recommend→fit(as_handle)→audit_result→sensitivity_from_result→bibtex`），也可按用户/复现要求切到 Stata（`00.2` `.do` pipeline）或 R（`00.3` tidyverse/fixest/Quarto pipeline）；再按设计配 `did-analysis` / `iv-estimation` / `rdd-analysis` / `synthetic-control` / `panel-data` / `ols-regression` / `time-series` / `ml-causal` + research-grade methods pack | `03_analysis/` 代码 + `design_register.md` + `method_gate.md` |
...
4. **一次性问清四件套**（用一个 `AskUserQuestion`，多选/多题，避免来回打断）：
...
与 [`references/analysis-backends.md`](references/analysis-backends.md)（Python/StatsPAI、Stata、R 三种后端选择），
...
5. **写入状态**：更新 `workflow_state.json.analysis_backend`、`method_gate` 与 `decisions`，记录分析后端、
   主设计、主估计量、缺失
   artifact、是否放行。
